        ...
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.asenajson"):
                    ...
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.polestarjson", "r").read())
                else:
            ...
            try:
                ...
                dil.delete()
                if path.isfile("./userbot/language/DEFAULT.polestarjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.polestarjson", "r").read())
                else:
                    raise Exception("Your language file is invalid")
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.polestarjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.polestarjson", "r").read())
        except JSONDecodeError:

...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @PolestarUserBot - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @PolestarUserBot - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/poyraz190321/AsenaUserbot.git")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@PolestarUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@PolestarUserBot Paketi")
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@PolestarUserBot"))
            bot(JoinChannelRequest("@PolestarSupport"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @PolestarUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Polestar açabilirsin; Kanala bak` @PolestarUserBot')
            else:
                await event.reply(f'`Tengri save hello I love you  Polestar... ❤`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@PolestarUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, CMD_HELP))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**❤  Merhaba Polestar Seni Seviyorum!** [Asena](https://t.me/AsenaUserBot) __Bot Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @PolestarUserBot",
                    text="""@PolestarUserbot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/PolestarUserbot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/PolestarSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/poyraz190321/PolestarUserbot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @PolestarUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**❤ Merhaba Polestar Seni Seviyorum!** [POLESTAR](https://t.me/PolestarUserBot) __Bot Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )

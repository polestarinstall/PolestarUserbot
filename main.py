...
try:
    ...
    idim = bot.get_me().id
    asenabl = requests.get('https://gitlab.com/poyraz190321/Polestar/-/raw/master/polestar.json').json()
    if idim in asenabl:
        bot.disconnect()
    ...
    PLUGIN_MESAJLAR = {}
    ORJ_PLUGIN_MESAJLAR = {"alive": "`Merhaba Polestar Seni Seviyorum!❤.`", "afk": f"`{str(choice(AFKSTR))}`", "kickme": "`Güle Güle ben gidiyorum `🤠", "pm": UNAPPROVED_MSG, "dızcı": str(choice(DIZCILIK_STR)), "ban": "`Yasaklandı!`", "mute": "`Sessize alındı!`", "approve": "`Bana mesaj gönderebilirsin!`", "disapprove": "`Artık bana mesaj gönderemezsin!`", "block": "`Engellendin!`"}
...
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          " Yardıma ihtiyacınız varsa, Destek grubumuza gelin t.me/PolestarSupport")
